...
import { assessScenario, STATUS_LABEL } from './scenario'
...
// Self-contained top-of-dashboard technical outlook: computes live indicators
// from 000660 and reads commentary/levels from the static outlook JSON.
export function ScenarioSection() {
  const { data: quote } = useQuote('000660.KS', '1Y')
  const { data: outlook } = useOutlook()

  const result = useMemo(() => {
    if (!quote?.intraday?.length) return null
    const candles: Candle[] = quote.intraday.map((p) => ({ t: p.t, price: p.price }))
  ...
  return (
    <div className="panel">
      <div className="panel-head" style={{ alignItems: 'baseline', flexWrap: 'wrap', gap: 6 }}>
        <div>
          <h2>
            SK하이닉스 기술적 전망 · 시나리오 보드
            <span className="badge" style={{ fontSize: 11, marginLeft: 8 }}>관찰 국면</span>
            <InfoTip text={TOOLTIPS.maBoll} />
          </h2>
          <div className="panel-sub">000660 실데이터 기반 · 강세/기준/약세 3시나리오 (투자자문 아님)</div>
        </div>
        <div className="scenario-meta">해설 기준 {format(new Date(outlook.updatedAt), 'yyyy.MM.dd HH:mm')} · 하루 2회 갱신</div>
      </div>

      <div className="disclaimer" role="note">⚠️ {outlook.disclaimer}</div>

      <div className="scenario-active-line">
        <span>해설(JSON) 활성: <strong>{KEY_LABEL[outlook.activeScenario]}</strong></span>
        <span>
          실시간 지표 판정:{' '}
          <strong style={{ color: 'var(--accent)' }}>{computedActive ? KEY_LABEL[computedActive] : '계산 대기'}</strong>
        </span>
      </div>
          ...
          return (
            <div key={s.key} className={`scenario-card ${s.key}${isActive ? ' active' : ''}`}>
              <div className="scenario-card-head">
                <span className="scenario-title">{s.emoji} {s.label}</span>
      ...
      {assessment && assessment.reasons.length > 0 && (
        <div className="scenario-reasons">현재 지표 근거: {assessment.reasons.join(' · ')}</div>
      )}
