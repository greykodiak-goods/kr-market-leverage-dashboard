...
export const PRESETS: Preset[] = [
  ...
  // ── 2026-08-03 대표 지시 "프리셋 추가해봐" — 32차 칼마 1위를 제품화한 것.
  //    이 화면에서 유일하게 **금(달러자산) 슬리브**가 붙는 프리셋이고, 그래서 곡선이
  //    2004-11부터 시작한다(GLD 상장). 다른 프리셋과 **구간이 다르므로 직접 비교하지 마라.**
  {
    id: 'calmar-max',
    label: '🏆 칼마최대형 — 결합+시장게이트+금20% (32차)',
    kind: 'combo',
    // 슬리브 A = 23차 수익÷MDD 1위와 **같은 스펙 객체**(combo-50·combo-25-75와 동일).
    spec: PRESET_PIT_MAXRATIO,
    mom: { slots: 5, gate: true },
    wA: 0.5,
    marketGate: true,
    goldW: 0.2,
    note:
      '2026-08-03 32차 실측(GHA idea:asset) — 슬리브 A(MA25×신고10→80선) + 슬리브 B(XSM 상위5+게이트, ' +
      '여기에 **시장게이트(12-1)**: 벤치 12-1 모멘텀이 음수인 달은 B 슬리브 전체를 현금)를 월초 50:50으로 되돌리고, ' +
      '그 곡선과 **금(GLD 원화) 20%**를 다시 월초 80:20으로 되돌리는 3자 결합. ' +
      '공통 구간(2004-11-18~2026-07-31) CAGR 23.1% · **MDD −23.8%** · 칼마 0.969 · 알파 +11.3%p/연 ' +
      '(전반 +10.2 / 후반 +11.8 — 양쪽 다 양수) [추정·러너 실행값]. ' +
      '⚠️ **곡선이 2004-11부터 시작한다** — 금(GLD)이 그때 상장했기 때문이다. 즉 이 성적에는 ' +
      '**닷컴 붕괴(2000~2003)가 통째로 빠져 있다.** 2000년부터 시작하는 다른 프리셋의 MDD·CAGR과 ' +
      '나란히 놓으면 그 비교는 거짓이다(겪은 위기의 수가 다르다). 남은 위기는 2008·2020·2022뿐이다. ' +
      '⚠️ **리밸런스 비용 미반영** — 주식↔금 이체(연 12회)와 해외 ETF 환전 스프레드가 0원으로 가정돼 있어 ' +
      '실제 성적은 이보다 낮다. 낙관적 상한으로 읽어라. ' +
      '⚠️ **화면 실행값은 32차 실측치와 소수점까지 일치하지 않는다** — 연구 러너는 시장게이트를 모멘텀 시뮬 ' +
      '안에서 걸어 게이트 달 첫날 시가에 청산(비용 발생)하지만, 화면은 이미 만들어진 B 곡선의 그 달 수익률을 ' +
      '0으로 만든다. 청산·재매수 비용과 첫날 갭이 빠지는 만큼 화면 쪽이 **조금 후하게** 나온다. ' +
      '⚠️ **달러 노출 의존** — 금 슬리브는 원화 곡선이라 금 가격과 원/달러 변동이 섞여 있다. 원/달러는 국내 증시와 ' +
      '역상관 경향이 있어(위기 때 원화 약세) 낙폭 완화의 상당 부분이 **금이 아니라 달러**일 수 있다. ' +
      '환헤지 상품을 쓰면 이 효과는 사라지고 결과가 달라진다. ' +
      '⚠️ **미 상장 ETF 기준이다** — 국내 대체품(KODEX 골드선물 등)은 상장일·보수·추적오차가 다르고, ' +
      '세제(해외주식 양도세 22% vs 국내 ETF 배당소득세)도 반영하지 않았다. ' +
      '⚠️ **금은 현금흐름이 없다** — 이자도 이익도 없어 과거 상관이 미래에 유지된다는 보장이 특히 약하다. ' +
      '⚠️ 25·26차(슬리브 선정) → 30차(오버레이) → 31차(재채점) → 32차(자산군)로 **이미 이 데이터에서 이긴 조합 ' +
      '위에 계속 변형을 얹어 온 누적 탐색의 끝**이다. 독립 실험이 아니므로 다중검정으로 부풀려진 성적일 위험이 ' +
      '겹겹이 쌓여 있고, 과최적화 위험은 이 화면의 어느 프리셋보다 크다. 생존편향(상폐 종목 가격 부재)도 그대로다. ' +
      '**매수 권유가 아니다**(규칙 4).',
  },
]
