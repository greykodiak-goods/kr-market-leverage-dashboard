 ...
 * 소스가 섞이는 구간의 고지.
 *
 * **2026-08-03(40차) 갱신**: 예전 문구는 "알파가 배당 반영 차이만큼 편향돼 있다"고 말했는데,
 * 그 편향은 그날 제거됐다(벤치·참고 벽을 전략과 같은 가격수익 기준으로 받는다).
 * 문구를 안 고치면 **없는 편향을 있다고 말하게 되고**, 읽는 사람이 알파를 약 2%p 깎아서 읽는다.
 * 규칙 3은 편향을 적으라는 것이지 없는 편향을 적으라는 것이 아니다.
 *
 * 지금도 남아 있는 것은 **여전히 야후 총수익인 자산 슬리브**(TLT·GLD)다 —
 * 그것은 비교 대상이 아니라 전략이 보유하는 자산이라 기준을 맞추지 않았다.
 */
export const MIXED_SOURCE_NOTE =
  '국내 유니버스 종목만 선택한 소스로 받습니다. 벤치마크(KODEX 200)와 참고선(QQQ·QLD·금·환율)은 ' +
  'KRX Open API가 주지 않는 종목·해외 자산이라 **계속 Yahoo**로 받습니다. ' +
  '다만 **알파를 재는 벤치와 참고 벽은 전략과 같은 기준(가격수익)으로 맞췄으므로**(2026-08-03 40차) ' +
  '배당 반영 차이로 인한 알파 편향은 없습니다 — 예전 회차처럼 알파를 배당수익률만큼 깎아 읽지 마십시오. ' +
  '남아 있는 혼재는 **자산 슬리브(TLT·금)가 여전히 총수익**이라는 점이며, 그 슬리브를 쓰는 변형에만 해당합니다.'
